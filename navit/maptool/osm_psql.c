...
map_collect_data_osm_db(char *dbstr, struct maptool_osm *osm)
{
	...
	fprintf(stderr, "Cannot setup cursor 2222 for nodes: %s\n", PQerrorMessage(conn));
	...
	fprintf(stderr, "Cannot setup 4444 cursor for ways: %s\n", PQerrorMessage(conn));
	...
	for (;;) {
		...
		fprintf(stderr, "Cannot setup 33333 cursor for nodes: %s\n", PQerrorMessage(conn));
	...
	for (;;) {
		...
		fprintf(stderr, "continue 222 with %i ways\n", count);
		...
		fprintf(stderr, "continue 333 with %i ways\n", count);
		for (int i = 0 ; i < count ; i++) {
	...
	for (;;) {
		...
		fprintf(stderr, "Cannot setup 666 cursor for relations: %s\n", PQerrorMessage(conn));
