...
map_route_occluded_rect_new(struct map_priv *priv, struct map_selection *sel) {
    ...
    //map_dump_file(route_map.u.map, "route.txt"); //jdg
...
android_sensors(struct navit *nav, int sensor, float *x, float *y, float *z) {
    ...
    if (sensor == TYPE_ACCELEROMETER) {
        if (*x > 7.5) {
            ...
            dbg(lvl_debug,"orientation = ORIENTATION_LANDSCAPE");
        }
        if (*y > 7.5) {
            orientation = ORIENTATION_PORTRAIT;
            dbg(lvl_debug,"orientation = ORIENTATION_PORTRAIT");
        }
        if (*z > 7.5) {
            orientation = ORIENTATION_FLAT;
            dbg(lvl_debug, "orientation = ORIENTATION_FLAT");
        }
    }
    if ((orientation_old == ORIENTATION_FLAT) != (orientation == ORIENTATION_FLAT)) {
        ...
        flags_graphics.u.num=orientation == ORIENTATION_FLAT ? 0:10; // 8 = draw_mode_begin_clear, 2 = ??
    ...
    case ORIENTATION_FLAT:
        if (sensor == TYPE_MAGNETIC_FIELD) {
            yaw = (float)(atan2f(-*y,-*x) * 180 / M_PI + 180);
        }
        pitch=0;
        break;
    case ORIENTATION_LANDSCAPE:
        if (sensor == TYPE_ACCELEROMETER) {
            pitch = (float)(atan2f(*x,*z) * 180 / M_PI);
        }
        if (sensor == TYPE_MAGNETIC_FIELD) {
            yaw = (float)(atan2f(-*y,*z) * 180 / M_PI + 180);
        }
        break;
    case ORIENTATION_PORTRAIT:
        if (sensor == TYPE_ACCELEROMETER) {
            pitch = (float)(atan2f(*y,*z) * 180 / M_PI);
        }
        if (sensor == TYPE_MAGNETIC_FIELD) {
            yaw = (float)(atan2f(*x,*z) * 180 / M_PI + 180);
        }
        break;
    ...
    if (navit_get_attr(nav, attr_transformation, &attr, NULL)) {
        ...
        if (sensor == TYPE_ACCELEROMETER) {
            ...
            navit_draw(nav); // telkens redraw doen ?!!
            dbg(lvl_info,"pitch %d %f",orientation,pitch);// todo: set level back to debug
        } else {
            struct attr attr;
            attr.type=attr_orientation;
            attr.u.num = (long)(yaw - 1.0);
            if (attr.u.num < 0)
                attr.u.num+=360;
            pedestrian_data.yaw=attr.u.num;
            navit_set_attr(nav, &attr);
            dbg(lvl_info,"yaw %d %f",orientation,yaw);// todo: set level back to debug
            if (orientation == ORIENTATION_FLAT)
...
pedestrian_navit_init(struct navit *nav) {
    ...
    flags_graphics.type = attr_flags_graphics;
    flags_graphics.u.num = 10; // 8 = draw_mode_begin_clear, 2 = ??
    navit_set_attr(nav, &flags_graphics);
    if (navit_get_attr(nav, attr_graphics, &graphics, NULL)) {
        struct attr attr;
        ...
        //todo: the camera should only be on when the device is vertical
        attr.u.num = 1;
