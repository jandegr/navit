...
<xsl:transform version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xi="http://www.w3.org/2001/XInclude">
   <xsl:template match="/config/navit/osd[1]">
      ...
      <xsl:text>&#x0A;        </xsl:text>
      <osd type="speed_warner" enabled="yes" speed_exceed_limit_offset = "5" x="{round(80*number($OSD_SIZE))}" y="{round(-80*number($OSD_SIZE))}" w="{round(80*number($OSD_SIZE))}" h="{round(80*number($OSD_SIZE))}" font_size="{round(300*number($OSD_SIZE))}" osd_configuration="1"/>
      <xsl:text>&#x0A;        </xsl:text>
      ...
      <xsl:text>&#x0A;        </xsl:text>
      <osd type="button" src="cursor_{number($ICON_BIG)}_{number($ICON_BIG)}.png" command="follow=0;set_center_cursor()" x="0" y="{round(60*number($OSD_SIZE))}" enable_expression="follow>1"/>
      <xsl:text>&#x0A;        </xsl:text>
