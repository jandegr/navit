...
//		ACRA.getErrorReporter().setEnabled(false);
		...
		//http://stackoverflow.com/questions/32345768/cannot-resolve-method-setlatesteventinfo
...
//		Log.e("Navit", "Navit -> pixels x=" + width_ + " pixels y=" + height_);
		Log.i("Navit", "Navit -> dpi=" + densityDpi);
		Log.i("Navit", "Navit -> density=" + Navit.metrics.density);
		Log.i("Navit", "Navit -> scaledDensity=" + Navit.metrics.scaledDensity);
		...
		{
			Log.w("Navit", "found device of very high density ("+densityDpi+")");
			Log.w("Navit", "using xxxhdpi values");
			my_display_density = "xxxhdpi";
		}
	...
	// seperate thread version	
//		cWorkerThread = new CWorkerThread(this, NavitLanguage, android.os.Build.VERSION.SDK_INT, my_display_density, NAVIT_DATA_DIR+"/bin/navit",map_filename_path);
//		cWorkerThread.start();
//		if(nativeThread) 
//		{
//			while(cWorkerThread.nativeRunning==false)
//			{
//				try {
//					Thread.sleep(100);
...
//		}
	...
	public void onResume()
	{
		...
		if (startup_intent != null)
		{
			...
			{
				Log.i("Navit", "**2**A " + startup_intent.getAction());
				Log.i("Navit", "**2**D " + startup_intent.getDataString());
			...
			if (geoString.matches("^[+-]{0,1}\\d+(|\\.\\d*),[+-]{0,1}\\d+(|\\.\\d*)$")) {
				...
				if (geo.length == 2) {
					try {
						...
						msg.setData(b);
						msg.sendToTarget();
						Log.d("Navit", "target found (b): " + geoString);
